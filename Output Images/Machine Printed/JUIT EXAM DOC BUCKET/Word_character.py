...
def get_words(img_path,img_id):
    ###########################CANNY EDGE DETECTION ###################################

    global new_path

    img = cv2.imread(img_path+'//line'+str(img_id)+'.png')  ;        print(img.shape)
    img = cv2.resize(img, (1465,1737)) ; print(img.shape)

    if not os.path.exists(img_path+'//line'+str(img_id)):
    	os.makedirs(img_path+'//line'+str(img_id))

    os.chdir(img_path+'//line'+str(img_id))
    new_path= os.getcwd()
    ...
    print("No of text words detected are :"+str(len(contours)))
    ...
    for npaContour in contours:
        ...
        if not os.path.exists(new_path+'//word'+str(it)):
            os.makedirs(new_path+'//word'+str(it))
        cv2.imwrite(new_path+'//word'+str(it)+'//word.png', imgROI)
        it+=1    
    ###################################################################################
...
if __name__ == "__main__":
	...
	for it in range(Max_line_number):
		line_path= new_cwd
		word_number_bucket.append(get_words(img_path=line_path,img_id=it+1))

	Total_words=0

	for val in word_number_bucket:
		Total_words+=val
