...
def main():
    ...
    imgGray = cv2.cvtColor(
        imgTrainingNumbers, cv2.COLOR_BGR2GRAY
    )  # get grayscale image
    ...
    for npaContour in npaContours:  # for each contour
        if (
            cv2.contourArea(npaContour) > MIN_CONTOUR_AREA
    ...
    np.savetxt(
        "classifications.txt", npaClassifications
