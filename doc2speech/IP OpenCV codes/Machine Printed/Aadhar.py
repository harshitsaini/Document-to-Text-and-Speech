...
src_path = "C://Users//Harshit//Desktop//Initial_img//Machine Printed//Aadhar//"
original = src_path + "Aadhar1.jpg"
import pytesseract
...
def getThresholded(img, smooth_it):
    ...
    threshed = cv2.adaptiveThreshold(
        img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 121, 2
    )
...
def get_string(img_path):
    ...
    opened = getOpened(conditioned, 150, 10)
    final = opened
    cv2.imwrite(src_path + "final.png", final)
    # img_show(img,conditioned,opened)
    print("Conditioning Done !")
    result = pytesseract.image_to_string(
        Image.open(src_path + "final.png"), lang="eng", config=tessdata_dir_config
    )
    # os.remove(temp)
    return result
...
print(get_string(src_path + "Aadhar1.jpg"))
print("-------DONE-------")
