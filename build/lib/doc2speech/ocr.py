...
def edgeDetection(img, output_path):
    ...
    opened = utilities.getOpened(thresholded, iterations=1, kernel_size=3)
...
def getBlocks(image_path, output_path):
    ...
    conditioned = utilities.getMorph(
        img=edges, iterations=5, kernel_size=7, erode_it=False
    )

    final = conditioned.copy()

    # Generation of contours
    contours, a = cv2.findContours(final, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for idx, npaContour in enumerate(contours):
        ...
        img = cv2.rectangle(
            img, (intX, intY), (intX + intW, intY + intH), (0, 0, 255), 1
        )
...
def performRecognition(doc_path, output_path, docName):
    ...
    ltn = getBlocks(image_path=image_path, output_path=output_path)
    ...
    for it in range(ltn):
        ...
        answer.append(getLines(join(new_output_path, "block.png"), new_output_path))
    ...
    with open(
        join(output_path, docName.strip(".png").strip("jpg").strip(".tif") + ".txt"),
        "w",
    ) as file:
        for data in answer:
            file.write(data)
